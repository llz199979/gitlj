      ...
      >
       <el-form-item label="考勤名称" prop="name">
        ...
        <el-form-item>
          <el-button
            type="primary"
            @click="addStudentClock('addStudentClock', attendence_type)"
          >
            确 定
          </el-button>
...
//       },
  ...
  computed: {
    ...
    get_StudentClockInList() {
      return this.$store.getters.get_StudentClockInList;
    }
  },
  mounted () {
  ...
  methods: {
    ...
    addStudentClock(formName, item) {
      let that = this;
      this.$refs[formName].validate((valid) => {
        if (valid) {
          that.attendVisible = false;
          this.$store.dispatch('addStudentClockIn', item)
          .then(() => {
     ...
     updateTeacher(form, teachers) {
      this.$refs[form].validate(valid => {
        if(valid) {
          this.$store.dispatch('updateStudentClock', teachers)
           .then(() => {
    ...
    deleteBatch() {     
        this.$confirm("此操作将删除该通知，是否继续？", "警告", {
          ...
          .then(() => {
            ...
            this.$store.dispatch("deleteStudentClockBatch", idLists);
          })
          .catch(() => {});   
    },
    setData (rule, data) {
