  ...
  >
    <div id="come" class="animated fadeIn">
      ...
      <template slot-scope="scope">
        <viewer>
          <img :src="scope.row.photoUrl" style="width:50px;height:50px;" srcset="">
         </viewer>
        </template>
       </el-table-column>
          <el-table-column align="center" label="照片介绍" prop="detail">
    ...
    <!-- 相册添加 -->
     <el-dialog title="相册添加" :visible.sync="honorVisible" :modal="false" width="500px">
    ...
    closeClick(i) {
     ...
     classChange(classId) {
      if(classId) {
        ...
        this.$store.dispatch('getClassStudent_')
        this.$store.dispatch('getClassPictures')//渲染班级作品
        .then(res => {
...
    },
    deleteBatch() {
          this.$confirm('此操作删除学校的所有数据，是否继续？', '警告', {
          ...
          center: true
            ...
            this.$store.dispatch('delClassPictureByIds',school)
          }).catch(() => {
     ...
     deletes(id) {
          this.$confirm('此操作将删除此照片，是否继续？', '警告', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
          center: true
